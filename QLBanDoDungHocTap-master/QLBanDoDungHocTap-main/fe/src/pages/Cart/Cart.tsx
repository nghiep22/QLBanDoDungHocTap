...
export const Cart = () => {
  ...
  const userId = nguoiDung?.taiKhoan_Id ?? Number(localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user') || '{}')?.taiKhoan_Id : NaN);
  ...
  const datHang = async (e: React.FormEvent) => {
    ...
    try {
      await dichVuApi.taoDonHang({
        khachHang_Id: Number.isFinite(userId) ? userId : undefined,
