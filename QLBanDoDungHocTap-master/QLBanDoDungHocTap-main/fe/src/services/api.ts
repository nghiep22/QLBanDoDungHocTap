...
class DichVuApi {
  ...
  async layDonHangCuaToi(): Promise<DonHangAPI[]> {
    const response = await fetch(`${URL_API_GOC}/api/donhang/me`, {
      headers: this.taoHeaders(true),
    });
  ...
  async layDanhSachKhachHang(search?: string): Promise<KhachHangAPI[]> {
    ...
    const url = params.toString() ? `/api/khachhang?${params}` : '/api/khachhang';
    const response = await fetch(`${URL_API_GOC}${url}`, {
      headers: this.taoHeaders(true),
    });
  ...
  async taoKhachHang(data: TaoKhachHangRequest): Promise<{ id: number }> {
    const response = await fetch(`${URL_API_GOC}/api/khachhang`, {
      method: 'POST',
      headers: this.taoHeaders(true),
      body: JSON.stringify(data),
    });
    ...
    if (!response.ok) {
      let thongBaoLoi = noiDung || 'Không thể tạo khách hàng';
      try {
        const error = JSON.parse(noiDung);
        thongBaoLoi = error.message || thongBaoLoi;
      } catch {
        // Noi dung khong phai JSON, giu nguyen text server tra ve.
      }
      throw new Error(thongBaoLoi);
    }
  ...
  async capNhatKhachHang(id: number, data: CapNhatKhachHangRequest): Promise<void> {
    const response = await fetch(`${URL_API_GOC}/api/khachhang/${id}`, {
      method: 'PUT',
      headers: this.taoHeaders(true),
      body: JSON.stringify(data),
    });
    ...
    if (!response.ok) {
      const noiDung = await response.text();
      let thongBaoLoi = noiDung || 'Không thể cập nhật khách hàng';
      try {
        const error = JSON.parse(noiDung);
        thongBaoLoi = error.message || thongBaoLoi;
      } catch {
  ...
  async xoaKhachHang(id: number): Promise<void> {
    const response = await fetch(`${URL_API_GOC}/api/khachhang/${id}`, {
      method: 'DELETE',
      headers: this.taoHeaders(true),
    });
    ...
    if (!response.ok) {
      const noiDung = await response.text();
      let thongBaoLoi = noiDung || 'Không thể xóa khách hàng';
      try {
        const error = JSON.parse(noiDung);
        thongBaoLoi = error.message || thongBaoLoi;
      } catch {
